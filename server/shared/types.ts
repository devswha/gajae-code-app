...
export interface AuthenticatedWebSocketUser { userId?: string | number; id?: string | number; username?: string; [key: string]: unknown; }
...
export interface ProviderModelOption { label: string; value: string; group?: string; description?: string; roles?: Partial<Record<ProviderRole, string>>; effort?: ModelEffort; }
...
export type GatewayEventKind = 'chat_subscribed' | 'session_upserted' | 'loading_progress' | 'protocol_error';
export type ServerEventKind = GatewayEventKind | MessageKind;
...
export type CreateProjectPathOutcome = 'created' | 'reactivated_archived' | 'active_conflict';
