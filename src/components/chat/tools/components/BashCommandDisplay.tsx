...
export const BashCommandDisplay: React.FC<BashCommandDisplayProps> = ({
  ...
  return (
    ...
    >
      ...
      {description && !open && (
        <div className="truncate px-2.5 pb-1.5 pl-[2.4rem] text-[11px] text-muted-foreground/70 italic">
          {description}
        </div>
      )}
      ...
      {open && hasOutput && (
        <div className="settings-content-enter border-t border-border">
          {description && (
            <div className="px-3 pt-2 text-[11px] text-muted-foreground/70 italic">{description}</div>
          )}
          ...
          <pre
            className={cn(
              'max-h-80 overflow-auto px-3 py-2 font-mono text-xs leading-relaxed break-all whitespace-pre-wrap',
