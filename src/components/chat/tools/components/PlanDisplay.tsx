  ...
  return <Collapsible defaultOpen={defaultOpen}>
    <Card className="my-1 flex flex-col rounded-lg border border-border bg-card shadow-none">
      <CardHeader className="flex flex-row items-start justify-between space-y-0 px-4 pt-4 pb-0">
        ...
        <CollapsibleTrigger className="inline-flex h-8 w-8 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground"><ChevronsUpDown className="h-4 w-4" /><span className="sr-only">{t('plan.toggle')}</span></CollapsibleTrigger>
      </CardHeader>
      <CollapsibleContent><CardContent className="px-4 pt-3 pb-4">
        {content ? <MarkdownContent content={content} className="prose prose-sm max-w-none text-muted-foreground" /> : isStreaming ? <div className="py-2"><Shimmer>{t('plan.generating')}</Shimmer></div> : null}
