...
export type GoalSupervisor = {
  ...
  controlGoal(runId: string, scope: GjcGoalScope, command?: GjcGoalCommand): Promise<GjcGoalSnapshot>;
};
...
export async function handleChatGoal(
  ...
  const scope = { appSessionId: row.session_id, cwd: await realpath(row.project_path), owner: `${typeof userId}:${userId}` };
  const run = chatRunRegistry.getRun(row.session_id);
  const active = run?.status === 'running' ? run : undefined;
  const runId = active?.writer.getAbortHandle() ?? null;
  ...
  if (active) {
    ...
    return supervisor.controlGoal(runId, scope, command);
  }
  const snapshot: GjcGoalSnapshot = row.provider_session_id
    ? await supervisor.inspectGoal(scope, row.provider_session_id)
