...
export function useChatComposerState(args: UseChatComposerStateArgs) {
  ...
  const optionsFor = useCallback((text: string): QueuedSendOptions => { let toolsSettings: any = { allowedTools: [], disallowedTools: [], skipPermissions: false }; try { const saved = safeLocalStorage.getItem('gjc-tools-settings'); if (saved) toolsSettings = JSON.parse(saved); } catch (error) { console.error('Error loading tools settings:', error); } return { model: gjcModel, effort: reasoningEffort, permissionMode: 'default', toolsSettings, skipPermissions: toolsSettings?.skipPermissions || false, sessionSummary: sessionLabel(selectedSession, text) }; }, [gjcModel, reasoningEffort, selectedSession]);
