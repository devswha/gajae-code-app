...
export const SubagentContainer: React.FC<SubagentContainerProps> = ({ toolInput, toolResult, subagentState, defaultOpen = false, showHistory = true, openOnFailure = true }) => {
  ...
  const status = failed ? 'error' : isComplete ? 'completed' : 'running';
  const activeSummary = activeTool ? toolSummary(activeTool.toolName, activeTool.toolInput) : '';
  const outcome = isComplete
    ? `${failed ? 'Failed' : 'Completed'} (${childTools.length} ${childTools.length === 1 ? 'tool' : 'tools'})`
    : null;
  // The folded row carries the outcome itself when the level hides everything
  // else, so a compact transcript still says whether the task succeeded.
  const outcomeInRow = !showHistory && outcome
    ? <span className={`text-[11px] ${failed ? 'text-destructive' : 'text-muted-foreground/70'}`}>{outcome}</span>
    : undefined;
  ...
  return <div className="my-1 border-l border-border py-0.5 pl-3">
    <CollapsibleSection title={input.description || 'Running task'} toolName={`Subagent / ${input.subagent_type || 'Agent'}`} open={defaultOpen || (failed && openOnFailure)} badge={<ToolStatusBadge status={status} />} action={outcomeInRow}>
      ...
      {outcome && <div className={`mt-1 flex items-center gap-1.5 text-xs ${failed ? 'text-destructive' : 'text-muted-foreground'}`}>
        <ToolStatusBadge status={status} />
        <span>{outcome}</span>
      </div>}
      {showHistory && childTools.length > 0 && <Collapsible className="mt-2" defaultOpen={defaultOpen}>
